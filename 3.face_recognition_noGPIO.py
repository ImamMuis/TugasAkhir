...
Fitur        :  1. Face Tracking dengan Servo
                ...
                5. Perekeman Video Monitoring Lingkungan sekitar
                6. Ambil foto terbaru dari webcam via Telegram Bot
                7. Pengiriman hasil pendeteksian wajah via Telegram Bot
"""
import os
import cv2
import time
...
teleBot_PWD = root +  'Mkt2k21'
internetTest = "http://www.google.com"
    ...
    if succes:
            ...
            if jumlahWajah > 1:
                ...
                if recordFlag == False:
                    if cX < facePosition[0]:
                        servoX_Degree += 1
                        servoMove(1, servoX_Degree)
                        imgRGB = cv2.putText(imgRGB, 'Wajah terlalu kiri!', (15, 50), 
                                             font, 0.7, (54, 67, 244), 2)
                
                    elif cX > facePosition[1]:
                        servoX_Degree -= 1
                        servoMove(1, servoX_Degree)
                        imgRGB = cv2.putText(imgRGB, 'Wajah terlalu kanan!', (15, 75), 
                                             font, 0.7, (54, 67, 244), 2)
                        
                    if cY < facePosition[2]:
                        servoY_Degree -= 1
                        servoMove(0, servoY_Degree)
                        imgRGB = cv2.putText(imgRGB, 'Wajah terlalu atas!', (15, 50), 
                                             font, 0.7, (54, 67, 244), 2)
                        
                    elif cY > facePosition[3]:
                        servoY_Degree += 1
                        servoMove(0, servoY_Degree)
                        imgRGB = cv2.putText(imgRGB, 'Wajah terlalu bawah!', (15, 75), 
                                             font, 0.7, (54, 67, 244), 2)
        ...
        if countFaceCompare == faceCompare:
            ...
            if DetectedFace_Last == 0 and DetectedFace_Now == 1:
                ...
                if TimeBetween > DetectedFace_Tolerance:
                    faceState = True
                    Date = getCurrent('DATE')
                    Time = getCurrent('Time')
                ...
                if TimeBetween > DetectedFace_Tolerance and faceState == True:
                    faceState = False
                    countFaceCompare = 0
                    print('Tidak Ada Wajah!')
                    telebotText = f'Tidak ada wajah dalam {DetectedFace_Tolerance} detik terakhir'
                    bot.sendMessage(chat_id, telebotText)
...
try:
    ...
    print('Program Stop')
    bot.sendMessage(chat_id, 
                    'Sistem Face Recognition terhenti dari PC')
